...
<template>
  <div class="order-container">
    <el-tabs @tab-change="tabChange" v-model="orderStatus">
      <!-- tab切换 -->
        ...
        <div v-else>
          ...
          <div class="order-item" v-for="order in orderList" :key="order.id">
            <div class="head">
              ...
              <!-- 未付款，倒计时时间还有 -->
              <!-- <span class="down-time" v-if="order.orderStatus === 0">
...
<script setup>
    ...
    // 获取订单列表
    const orderList = ref([])
    const pageNum = ref(1), pageSize = ref(10), orderStatus = ref()
    const total = ref(0)
    const getOrderList = async() => {
      const res = await getUserOrder(pageNum.value, pageSize.value, orderStatus.value)
